...
// ESP32-C3 物理引脚直接映射（移除 ESP8266 专用的 D6/D7 定义，改用 GPIO 6 和 GPIO 7）
constexpr uint8_t TM1637_CLK_PIN = 6;  // CLK → GPIO6
constexpr uint8_t TM1637_DIO_PIN = 7;  // DIO → GPIO7
...
void display_update(float voltage, float current_mA, bool userEnabled, bool forceUpdate = false) {
    ...
    if (now - lastSegWriteMs >= refreshInterval || modeChanged || forceUpdate) {
        ...
        if (currentDispMode == DISP_TIME) {
            ...
            // 电压显示（双位整数+1位小数）：如 "12.5U" 或 "12:5U"
            ...
            // 电流显示：保留一位小数，整体向右平移 1 位，对齐物理冒号位置
            int amp_10 = (int)((current_mA + 50.0f) / 100.0f); 
            if (amp_10 > 99) amp_10 = 99; // 限制最高显示 9.9A

            showData[0] = 0;                                   // 第一位留空
            showData[1] = getDigitSegment(amp_10 / 10) | 0x80; // 第二位（个位）带点
