...
// =================================================================================================
// 描述: 本脚本专为ESP8266设计，用于12V锂电池供电系统。它通过INA219监测电压电流，
//       控制一个继电器，并提供网页界面进行手动控制、参数设置和OTA固件更新。
//
// 主要功能:
// 1. INA219电量监测：实时显示电压、电流、功率。
// 2. 网页控制：通过Web界面手动开关继电器。
// 3. 智能电压保护：带1小时锁定及手动覆盖功能。
// 4. 7日电量统计：以条形图直观显示最近7天的用电量。
// 5. 月度电量与电费统计：实时显示当月总用电量和预估电费。
// 6. 网页OTA更新：可通过网页上传新固件。
//
// v2.3 更新日志 (在 v2.2 基础上):
// - 新增功能: 在7日统计下方增加了“本月统计”模块，包含“本月总用电量 (kWh)”和“预估电费 (元)”。
// - 新增功能: 在网页设置中增加了“电价”输入框，用于计算电费，设置会永久保存。
// - 核心优化: 增加了月度数据自动结转功能，当月份变更时，月度统计会自动清零。
// - 核心优化: 所有新增加的设置和数据（电价、月度用电量）都会保存到EEPROM，确保重启不丢失。
// =================================================================================================
...
// ============== EEPROM 存储地址定义 ==============
const int EEPROM_SIZE = 256; // 扩容以存储更多数据
const int ADDR_MAGIC_NUM   = 0;  // 2字节
const int ADDR_LOW_V       = 2;  // 4字节
const int ADDR_HIGH_V      = 6;  // 4字节
const int ADDR_TODAY_WH    = 10; // 4字节
const int ADDR_LAST_DAY    = 14; // 1字节
const int ADDR_HISTORY_WH  = 15; // 6 * 4 = 24字节 (存储过去6天)
// --- 新增月度统计地址 ---
const int ADDR_PRICE_PER_KWH = 40; // 4字节 (电价)
const int ADDR_MONTH_WH      = 44; // 4字节 (本月用电量)
const int ADDR_LAST_MONTH    = 48; // 1字节 (上个月份)
const uint16_t EEPROM_MAGIC_NUMBER = 0x1A2D; // 更新魔法数字以强制重置数据结构
...
float dailyEnergyHistory[6] = {0}; // 存储昨天到6天前的用电量
unsigned long lastEnergyCalcMs = 0;
int lastDayChecked = -1;

// --- 新增月度电费统计变量 ---
float pricePerKwh = 0.68; // 默认电价 0.68元/度
...
void accumulateEnergy() {
  if (!ina219_ok || !relayState) { lastEnergyCalcMs = millis(); return; }
  ...
  if (elapsedMs > 0) {
    ...
    currentMonthEnergyWh += energyIncrementWh; // 同步增加到月度统计
  }
...
void checkDataRollover() {
  ...
  if (lastMonthChecked != currentMonth) {
    ...
    currentMonthEnergyWh = 0.0; // 月度用电量清零
    lastMonthChecked = currentMonth;
    saveEnergyData(); // 保存清零后的月度数据
  }
  ...
  if (lastDayChecked != currentDay) {
    ...
    // 数据滚动：将历史数据向后移动一位
    for (int i = 5; i > 0; i--) {
      dailyEnergyHistory[i] = dailyEnergyHistory[i - 1];
    }
    // 将“昨天”的数据（即旧的“今天”）存入历史记录的第一位
...
void setup() {
  ...
  if (!ina219.begin()) {
    Serial.println("[!!] 硬件错误: 未能找到INA219芯片!");
    ina219_ok = false;
  } else {
  ...
  server.on("/getMonthlyStats", HTTP_GET, handleGetMonthlyStats); // 新增路由
