...
static bool _postNote(const char* targetDate, unsigned long runSec, double cumulativeWh) {
  ...
  client.setInsecure(); // 绕过 HTTPS 证书链校验
  ...
  // Markdown 表格输出：| 时间 | IP | 昨天开风扇时间 | 累计功率 | [2]
  char tableRow[256];
...
// 初始化打卡标志
void note_begin() {
  // ESP32 特有：LittleFS.begin(true) 参数可在挂载失败时执行自动格式化
  if (!LittleFS.begin(true)) {
    Serial.println("[DailyNote] LittleFS 挂载失败！");
  }
