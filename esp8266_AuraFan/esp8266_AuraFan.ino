// ================== ESP8266 智能风扇 & LED 控制器 v1.6 ==================
//
// 更新日志 (v1.6):
// 1. [BUG修复] 解决了 'jump to case label crosses initialization' 编译错误。
//    通过为 switch-case 块添加花括号来创建独立作用域。
// 2. [配置更新] 根据用户要求，将两条灯带的物理灯珠数均配置为 60。
//
// 项目名称: AuraFan
// =======================================================================
...
// ==================== LED 灯带配置 (核心区域) ====================
// [v1.6] 更新: 两条灯带均为 60 灯珠
#define NUM_STRIPS 2 // <--- 修改这里的灯带总数量
...
void updateLeds() {
    ...
    #if NUM_STRIPS >= 1
    {
        ...
        int num_active = min(numLedsToLight, STRIP1_LEDS);
        switch (currentLedMode) {
            case STATIC: {
                fill_solid(strip_leds, num_active, staticColor);
                break;
            }
            case RAINBOW: {
                fill_rainbow(strip_leds, num_active, gHue, 7);
                break;
            }
            case FADE: { // <<<< 修复点: 添加花括号
    ...
    {
        CRGB* strip_leds = &leds[offset];
        int num_active = min(numLedsToLight - STRIP1_LEDS, STRIP2_LEDS);
        if (num_active < 0) num_active = 0;
