...
#include <WebServer.h> // 【修改】使用 ESP32 通用 WebServer
#include <time.h>
#include <LittleFS.h>
...
private:
  ...
  WebServer*          webServer; // 【修改】使用通用 WebServer
  ...
  void updateDisplay() {
    ...
    // 【修改】ESP32-C3 的 RMT/I2S 驱动无阻塞极快，直接输出，去除 ESP8266 硬件 WDT 开关
    matrix->show();     
    yield();
  }
...
public:
  SmartMatrixClock(int pin, WebServer& srv) { // 【修改】WebServer 引用更改
    clockPin  = pin;
    webServer = &srv;
